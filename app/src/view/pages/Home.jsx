...
export default function Home() {
  ...
  useEffect(() => {
    try {
      Promise.all([
        numberChatsNotReading(context.token),
    ...
    <div className="w-full h-full">
        ...
        {page === "Profile" ? (
          <nav>
            <button
              ...
              className="bg-color4 text-white border-none rounded-xl px-3 py-1 mr-3 font-bold text-lg cursor-pointer transition duration-300 hover:bg-color3"
            >
              Logout
            </button>
          </nav>
      ...
      <footer className="w-full h-16 bg-color5 fixed bottom-0 left-0 flex justify-around items-center">
        ...
        <Link
          ...
          className="text-white text-2xl mx-2 no-underline border-b-2 border-transparent transition-transform duration-200 hover:scale-125"
          to={`/profile/${userIdProfile}/posts`}
        >
          {user && (
            <img
      ...
      {modal === "create-post-modal" && (
        <CreatePostModal
          ...
          onHideCreatePost={handleCancelCreatePostModal}
        />
      )}
